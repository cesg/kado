      ...
      >
        ...
        <p style="margin: 0 0.5rem">
          <span
            class="tag"
            title="Tasks"
            v-text="`${kanban.tasks_done}/${kanban.tasks_count}`"
          />
        </p>
        <p style="margin: 0 0.5rem">
          <span
...
const extractChecklist = body => {
  if (!body || !body.blocks) {
    return [];
  }

  return body.blocks.reduce((all, current) => {
    if (current.type != 'checklist') {
      return all;
    }
    return all.concat(current.data.items);
  }, []);
};
...
export default {
  ...
  methods: {
    async fetchKanbans() {
      const { data } = await Xhr.get('kanbans', { params: { step_id: 2, eager: '[size,assigned]' } });
      this.kanbans = data.map(item => {
        const body = item.body.length > 2 ? JSON.parse(item.body) : {};
        const tasks = extractChecklist(body);
        item.tasks_count = tasks.length;
        item.tasks_done = tasks.filter(({ checked }) => checked).length;
        return item;
      });
      this.fetchDone = true;
    },
